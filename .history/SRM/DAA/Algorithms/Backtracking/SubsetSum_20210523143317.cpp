...
void subset_sum(int s[], int [],int s_size, int t_size,int sum, int ite,int const target_sum){
   
    if (target_sum == sum ){
        PrintSubSet(, t_size);
        subset_sum(s, , s_size, t_size - 1,sum - s[ite], ite + 1,target_sum);
        return;
    }
    else{
        for(int i = ite; i < s_size; i++){
            ...
            subset_sum(s, , s_size, t_size + 1,sum + s[i], i + 1, target_sum);
        }
    }
}
