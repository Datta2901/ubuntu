...
void subset_sum(vector<int> , vector<int> result,int s_size, int t_size,int sum, int ite,int const target_sum){
   
    if (target_sum == sum ){
        PrintSubSet(result, t_size);
        subset_sum(, result, s_size, t_size - 1,sum - [ite], ite + 1,target_sum);
        return;
    }
    else{
        for(int i = ite; i < s_size; i++){
            ...
            subset_sum(, result, s_size, t_size + 1,sum + [i], i + 1, target_sum);
        }
    }
}
