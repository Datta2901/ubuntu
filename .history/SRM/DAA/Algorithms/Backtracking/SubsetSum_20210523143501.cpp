...
void subset_sum(vector<int> temp,vector<int> result,int te, int t_size,int sum, int ite,int const target_sum){
   
    if (target_sum == sum ){
        PrintSubSet(result, t_size);
        subset_sum(temp, result, te, t_size - 1,sum - temp[ite], ite + 1,target_sum);
        return;
    }
    else{
        for(int i = ite; i < te; i++){
            result[t_size] = temp[i];
             
            subset_sum(temp, result, te, t_size + 1,sum + temp[i], i + 1, target_sum);
        }
    }
}
