...
void subset_sum(vector<int> temp,vector<int> result,int sum, int ,int const target_sum){
   
    if (target_sum == sum ){
        PrintSubSet(result);
        subset_sum(temp, result, temp.size(), result.size() - 1,sum - temp[],  + 1,target_sum);
        return;
    }
    else{
