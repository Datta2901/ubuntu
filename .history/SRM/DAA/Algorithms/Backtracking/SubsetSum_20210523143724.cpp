...
void subset_sum(vector<int> temp,vector<int> result,int temp_size,int result_sint sum, int pos,int target_sum){
   
    if (target_sum == sum ){
        PrintSubSet(result);
        subset_sum(temp, result, temp.size(), result.size() - 1,sum - temp[pos], pos + 1,target_sum);
        return;
    }
    else{
