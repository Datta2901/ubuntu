...
void isaValidCell(int i,int j,vector<vector<int>> &grid,string p,vector<int> &Path,vector<vector<int>> &cost,vector<int> &temp,int Cost,vector<pair<string,int>> &vect){
 ...
 void find_path(int i,int j,vector<vector<int>> &g,string s,int n,vector<string>&res,vector<vector<int>> &cost,vector<int> &ktk,int sum,     vector<pair<string,int>> &vect)
   {   //BASE CASE TO CHECK IF THE PATH GO'S OUT OF BOUND OR TO CHECK IF IT IS VISITED   OR IF THERE IS A WALL
       ...
       {
           g[i][j]=-1;
           find_path(i+1,j,g,s+'D',n,res,cost,ktk,sum+cost[i][j],vect);
           find_path(i-1,j,g,s+'U',n,res,cost,ktk,sum+cost[i][j],vect);
           find_path(i,j+1,g,s+'R',n,res,cost,ktk,sum+cost[i][j],vect);
           find_path(i,j-1,g,s+'L',n,res,cost,ktk,sum+cost[i][j],vect);
           g[i][j]=0;
       }
   }
