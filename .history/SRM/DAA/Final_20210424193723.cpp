...
void isaValidCell(int i,int j,vector<vector<int>> &grid,string p,vector<int> &Path,vector<vector<int>> &cost,vector<int> &temp,int Cost,vector<pair<string,int>> &vect){
 ...
 void find_path(int i,int j,vector<vector<int>> &gri,string s,int n,vector<string>&res,vector<vector<int>> &cost,vector<int> &ktk,int sum,     vector<pair<string,int>> &vect)
   {   //BASE CASE TO CHECK IF THE PATH GO'S OUT OF BOUND OR TO CHECK IF IT IS VISITED   OR IF THERE IS A WALL
       if(i<0 || j<0 || i>=n || j>=n )
       {
           return;
       }
       if( gri[i][j]==1 || gri[i][j]==-1)
       return;
       ...
       {
           gri[i][j]=-1;
           find_path(i+1,j,gri,s+'D',n,res,cost,ktk,sum+cost[i][j],vect);
           find_path(i-1,j,gri,s+'U',n,res,cost,ktk,sum+cost[i][j],vect);
           find_path(i,j+1,gri,s+'R',n,res,cost,ktk,sum+cost[i][j],vect);
           find_path(i,j-1,gri,s+'L',n,res,cost,ktk,sum+cost[i][j],vect);
           gri[i][j]=0;
       }
   }
