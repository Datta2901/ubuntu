...
void isaValidCell(int i,int j,vector<vector<int>> &grid,string p,vector<int> &Path,vector<vector<int>> &cost,vector<int> &temp,int Cost,vector<pair<string,int>> &vect){
    ...
    // If we reach top right corner (Destination)
        ...
        P.push_back(p);
        ktk.push_back(sum);
        vect.push_back(make_pair(p,sum));
        return ;
    }else{
        grid[i][j] = INT_MIN;
 ...
 void find_path(int i,int j,vector<vector<int>> &grid,string p,int n,vector<string>&P,vector<vector<int>> &cost,vector<int> &ktk,int sum,     vector<pair<string,int>> &vect)
   {   //BASE CASE TO CHECK IF THE PATH GO'S OUT OF BOUND OR TO CHECK IF IT IS VISITED   OR IF THERE IS A WALL
       ...
       {
           grid[i][j]=-1;
           find_path(i+1,j,grid,p+'D',n,P,cost,ktk,sum+cost[i][j],vect);
           find_path(i-1,j,grid,p+'U',n,P,cost,ktk,sum+cost[i][j],vect);
           find_path(i,j+1,grid,p+'R',n,P,cost,ktk,sum+cost[i][j],vect);
           find_path(i,j-1,grid,p+'L',n,P,cost,ktk,sum+cost[i][j],vect);
           grid[i][j]=0;
       }
   }
...
int main(){
    cout << "Enter row and column Ppectively\n";
    cin >> row >> column;
