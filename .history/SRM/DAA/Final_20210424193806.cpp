...
void isaValidCell(int i,int j,vector<vector<int>> &grid,string p,vector<int> &Path,vector<vector<int>> &cost,vector<int> &temp,int Cost,vector<pair<string,int>> &vect){
 ...
 void find_path(int i,int j,vector<vector<int>> &grid,string p,int n,vector<string>&Path,vector<vector<int>> &cost,vector<int> &te,int sum,     vector<pair<string,int>> &vect)
   {   //BASE CASE TO CHECK IF THE PATH GO'S OUT OF BOUND OR TO CHECK IF IT IS VISITED   OR IF THERE IS A WALL
       ...
       {
           grid[i][j]=-1;
           find_path(i+1,j,grid,p+'D',n,Path,cost,te,sum+cost[i][j],vect);
           find_path(i-1,j,grid,p+'U',n,Path,cost,te,sum+cost[i][j],vect);
           find_path(i,j+1,grid,p+'R',n,Path,cost,te,sum+cost[i][j],vect);
           find_path(i,j-1,grid,p+'L',n,Path,cost,te,sum+cost[i][j],vect);
           grid[i][j]=0;
       }
   }
