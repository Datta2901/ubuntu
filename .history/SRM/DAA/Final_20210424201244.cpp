...
void PossiblePaths(int i,int j,vector<vector<int> > &grid,string p,vector<int> &Path,vector<vector<int> > &cost,vector<int> &temp,int PaCost,vector<pair<string,int> > &Values){
    grid[i][j] = INT_MIN;
...
void GetPath(vector<vector<int>> &grid,vector<vector<int>> &cost){
    ...
    isValidPath(column-1,0,grid,"",Path,cost,temp,sum, Values);
    PrintResults(temp);
}
