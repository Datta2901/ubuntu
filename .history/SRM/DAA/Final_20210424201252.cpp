...
void PossiblePaths(int i,int j,vector<vector<int> > &grid,string p,vector<int> &Path,vector<vector<int> > &cost,vector<int> &temp,int PathCost,vector<pair<string,int> > &Values){
    grid[i][j] = INT_MIN;
    isValidPath(i+1,j,grid,p+'D',Path,cost,temp,+cost[i][j],Values);
    isValidPath(i-1,j,grid,p+'U',Path,cost,temp,+cost[i][j],Values);
    isValidPath(i,j+1,grid,p+'R',Path,cost,temp,+cost[i][j],Values);
    isValidPath(i,j-1,grid,p+'L',Path,cost,temp,+cost[i][j],Values);
    grid[i][j]=0;
}
...
void GetPath(vector<vector<int>> &grid,vector<vector<int>> &cost){
     ...
     int =0;
    // TO KEEP A TRACK OF THE  FOR RESPECTIVE PATH
    isValidPath(column-1,0,grid,"",Path,cost,temp,, Values);
    PrintResults(temp);
}
