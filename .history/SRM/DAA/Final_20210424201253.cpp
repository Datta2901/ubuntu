...
void PossiblePaths(int i,int j,vector<vector<int> > &grid,string p,vector<int> &Path,vector<vector<int> > &cost,vector<int> &temp,int PathCost,vector<pair<string,int> > &Values){
    grid[i][j] = INT_MIN;
    isValidPath(i+1,j,grid,p+'D',Path,cost,temp,P+cost[i][j],Values);
    isValidPath(i-1,j,grid,p+'U',Path,cost,temp,P+cost[i][j],Values);
    isValidPath(i,j+1,grid,p+'R',Path,cost,temp,P+cost[i][j],Values);
    isValidPath(i,j-1,grid,p+'L',Path,cost,temp,P+cost[i][j],Values);
    grid[i][j]=0;
}
...
void isValidPath(int i,int j,vector<vector<int>> &grid,string p,vector<int> &Path,vector<vector<int>> &cost,vector<int> &temp,int Cost,vector<pair<string,int>> &Values){
    ...
    // If we reach top right corner (Destination)
        ...
        Path.push_back(p);
        temp.push_back(P);
        Values.push_back(make_pair(p,P));
        return ;
    }else{
...
void GetPath(vector<vector<int>> &grid,vector<vector<int>> &cost){
     ...
     int P=0;
    //P TO KEEP A TRACK OF THE P FOR RESPECTIVE PATH
    isValidPath(column-1,0,grid,"",Path,cost,temp,P, Values);
    PrintResults(temp);
}
