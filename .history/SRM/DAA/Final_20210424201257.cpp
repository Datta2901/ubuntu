...
void PossiblePaths(int i,int j,vector<vector<int> > &grid,string p,vector<int> &Path,vector<vector<int> > &cost,vector<int> &temp,int PathCost,vector<pair<string,int> > &Values){
    grid[i][j] = INT_MIN;
    isValidPath(i+1,j,grid,p+'D',Path,cost,temp,PathC+cost[i][j],Values);
    isValidPath(i-1,j,grid,p+'U',Path,cost,temp,PathC+cost[i][j],Values);
    isValidPath(i,j+1,grid,p+'R',Path,cost,temp,PathC+cost[i][j],Values);
    isValidPath(i,j-1,grid,p+'L',Path,cost,temp,PathC+cost[i][j],Values);
    grid[i][j]=0;
}


void isValidPath(int i,int j,vector<vector<int>> &grid,string p,vector<int> &Path,vector<vector<int>> &cost,vector<int> &temp,int Cost,vector<pair<string,int>> &Values){
    ...
    // If we reach top right corner (Destination)
        ...
        Path.push_back(p);
        temp.push_back(PathC);
        Values.push_back(make_pair(p,PathC));
        return ;
    }else{
...
void GetPath(vector<vector<int>> &grid,vector<vector<int>> &cost){
     ...
     int PathC=0;
    //PathC TO KEEP A TRACK OF THE PathC FOR RESPECTIVE PATH
    isValidPath(column-1,0,grid,"",Path,cost,temp,PathC, Values);
    PrintResults(temp);
}
