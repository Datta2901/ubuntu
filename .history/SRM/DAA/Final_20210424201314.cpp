...
void isValidPath(int i,int j,vector<vector<int>> &grid,string p,vector<int> &Path,vector<vector<int>> &cost,vector<int> &temp,int ,vector<pair<string,int>> &Values){
    ...
    // If we reach top right corner (Destination)
        ...
        PossiblePaths(i,j,grid,p,Path,cost,temp,,Values);
    }
}
