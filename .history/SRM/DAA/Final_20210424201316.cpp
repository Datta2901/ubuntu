...
void isValidPath(int i,int j,vector<vector<int>> &grid,string p,vector<int> &Path,vector<vector<int>> &cost,vector<int> &temp,int PathCo,vector<pair<string,int>> &Values){
    ...
    // If we reach top right corner (Destination)
        ...
        PossiblePaths(i,j,grid,p,Path,cost,temp,PathCo,Values);
    }
}
...
int main(){
    ...
    cout << "Enter the PathCo of each cell in the grid\n";
    ...
    cout << "Given PathCo of each cell is :\n";
