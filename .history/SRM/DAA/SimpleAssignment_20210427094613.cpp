...
void isValidPath(int i,int j,vector<vector<int> > &grid,string p,vector<string> &Path,vector<vector<int> > &cost,int PathCost,vector<pair<string,int> > &Values){
    ...
    // If we reach top right corner (Destination)
        ...
        grid[i][j] = INT_MIN;
        isValidPath(i+1,j,grid,p+'D',Path,cost,PathCost+cost[i][j],Values);
        isValidPath(i-1,j,grid,p+'U',Path,cost,PathCost+cost[i][j],Values);
        isValidPath(i,j+1,grid,p+'R',Path,cost,PathCost+cost[i][j],Values);
        isValidPath(i,j-1,grid,p+'L',Path,cost,PathCost+cost[i][j],Values);
        // grid[i][j] = 0;
    }
}
