...
struct Node *quickSortRecursi(struct Node *head, struct Node *end)
{
    if (!head || head == end)
        return head;
    ...
    if (newHead != pivot)
    {
        ...
        newHead = quickSortRecursi(newHead, tmp);

        tmp = getTail(newHead);
        tmp->next = pivot;
    }

    pivot->next = quickSortRecursi(pivot->next, newEnd);
...
{
    (*headRef) = quickSortRecursi(*headRef, getTail(*headRef));
    return;
}
