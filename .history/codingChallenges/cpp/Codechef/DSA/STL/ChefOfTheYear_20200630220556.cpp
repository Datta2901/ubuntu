...
int main(){
    ...
    loop(m){
        cin >> a;
        ChefVotes[a]++;
        CountryVotes[origin[a]]++;
        if(ChefVotes[a] > maxChefVotes){
            maxChefVotes =  ChefVotes[a];
            maxChef = a;
        }else if(ChefVotes[a] == maxChefVotes){
            maxChef = min(a , maxChef);
        }if(CountryVotes[origin[a]] > maxCountryVotes){
            maxCountryVotes = CountyVotes[a];
            maxCountry = origin[a];
        }else if(CountryVotes[origin[a]] == maxCountryVotes){
            maxCountry = min(maxCountry,origin[a]);
        }
    }
