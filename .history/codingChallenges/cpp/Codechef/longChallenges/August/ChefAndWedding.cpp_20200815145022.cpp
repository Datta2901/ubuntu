...
long long int getArguments(vector<int> answer){
    map<int,int> freq;
    for(int i = 0; i < answer.size(); i++){
        freq[answer[i]]++;
    }
    for(auto ip = freq.begin(); ip != freq.end(); i )
}
...
int main(){
    ...
    testcase(){
       int size,cost;
        cin >> size >> cost;
        vector<int> guests,answer;
        for(int i = 0; i < size; i++){
            int a;
            cin >> a;
            guests.pb(a);
        }
        int count = 1,arguments = 0;
        answer.pb(guests[0]);
        for(auto it = guests.begin() + 1; it != guests.end(); it++){
            if(find(answer.begin(),answer.end(),*it) == answer.end()){
                ...
                answer.pb(*it);
                long long int result  = getArguments(answer);
                if(result < cost){
                    arguments += result;
                }else{
                    count++;
