...
#define endl '\n'
using namespace std;
#define vectorInput(vect,n)for(int i = 0;i < n; i++){ int a; cin >> a;vect[i] = 1;}
...
int main(){
   ...
   testcase(){
    ...
    vector<vector<ll> > arrangement(101,vector<ll> (1001,0));

    rep(n + 1,i){
        arrangement[1][i] = column[0][i - 1];
    }
    ...
    repeat(i,num,1){
        Ineffieciency = min(Ineffieciency,i * k + arrangement[i][n]);
    }
