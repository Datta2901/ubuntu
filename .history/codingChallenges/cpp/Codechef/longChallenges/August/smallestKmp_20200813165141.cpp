...
// EXECUTE_FASTER
testcase(){
    strinmain,pattern,answer,answer2;
    cin >> main >> pattern;
    main = common(main,pattern);
    main += pattern[0];
    sort(main.begin(),main.end()); 
    // cout << str << endl;
    int index = main.find(pattern[0]);
    int index2 = main.find_last_of(pattern[0]);
    ...
    answer2 += pattern;
    for(int i = index2 + 1; i < main.size(); i++){
        if(main[i] != '0'){
            answer2 += main[i];
        } 
    }
    ...
    answer += pattern;
    for(int i = index + 1; i < main.size(); i++){
        if(main[i] != '0'){
            answer += main[i];
        } 
    }
