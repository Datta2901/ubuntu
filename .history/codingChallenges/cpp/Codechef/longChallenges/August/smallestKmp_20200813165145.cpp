...
// EXECUTE_FASTER
testcase(){
    strinmainStin,pattern,answer,answer2;
    cin >> mainStin >> pattern;
    mainStin = common(mainStin,pattern);
    mainStin += pattern[0];
    sort(mainStin.begin(),mainStin.end()); 
    // cout << str << endl;
    int index = mainStin.find(pattern[0]);
    int index2 = mainStin.find_last_of(pattern[0]);
    ...
    answer2 += pattern;
    for(int i = index2 + 1; i < mainStin.size(); i++){
        if(mainStin[i] != '0'){
            answer2 += mainStin[i];
        } 
    }
    ...
    answer += pattern;
    for(int i = index + 1; i < mainStin.size(); i++){
        if(mainStin[i] != '0'){
            answer += mainStin[i];
        } 
    }
