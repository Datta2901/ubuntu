...
// EXECUTE_FASTER
testcase(){
    strinmainStein,pattern,answer,answer2;
    cin >> mainStein >> pattern;
    mainStein = common(mainStein,pattern);
    mainStein += pattern[0];
    sort(mainStein.begin(),mainStein.end()); 
    // cout << str << endl;
    int index = mainStein.find(pattern[0]);
    int index2 = mainStein.find_last_of(pattern[0]);
    ...
    answer2 += pattern;
    for(int i = index2 + 1; i < mainStein.size(); i++){
        if(mainStein[i] != '0'){
            answer2 += mainStein[i];
        } 
    }
    ...
    answer += pattern;
    for(int i = index + 1; i < mainStein.size(); i++){
        if(mainStein[i] != '0'){
            answer += mainStein[i];
        } 
    }
