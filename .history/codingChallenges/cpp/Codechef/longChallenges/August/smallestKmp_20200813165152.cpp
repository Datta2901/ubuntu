...
// EXECUTE_FASTER
testcase(){
    strinmainSt,pattern,answer,answer2;
    cin >> mainSt >> pattern;
    mainSt = common(mainSt,pattern);
    mainSt += pattern[0];
    sort(mainSt.begin(),mainSt.end()); 
    // cout << str << endl;
    int index = mainSt.find(pattern[0]);
    int index2 = mainSt.find_last_of(pattern[0]);
    ...
    answer2 += pattern;
    for(int i = index2 + 1; i < mainSt.size(); i++){
        if(mainSt[i] != '0'){
            answer2 += mainSt[i];
        } 
    }
    ...
    answer += pattern;
    for(int i = index + 1; i < mainSt.size(); i++){
        if(mainSt[i] != '0'){
            answer += mainSt[i];
        } 
    }
