...
int findThePosition(vector<int> numbers,int target,int p){
...
int main(){
   ...
   testcase(){
        ...
        int pos = findThePosition(numbers,target,p),answer = 0;
        if(numbers[pos] != target){
            numbers[k] == target;
            sort(numbers.begin(),numbers.end());
            answer++;
        }
        if(p > k && numbers[p] > target || (p < k && numbers[p] < target)){
            cout << -1 << endl;
            continue;
        }
        if(numbers[p] == target){
        ...
        pos = findThePosition(numbers,target,p);
