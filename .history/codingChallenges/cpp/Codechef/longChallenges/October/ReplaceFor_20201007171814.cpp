...
int findThePosition(vector<int> numbers,int target,int in){
...
int main(){
    EXECUTE_FASTER
   testcase(){
        int size,target,in,k;
        cin >> size >> target >> in >> k;
        in -= 1,k -= 1;
        vector<int>numbers;
        ...
        int pos = findThePosition(numbers,target,in),answer = 0;
        if(numbers[pos] != target){
            numbers[k] == target;
            sort(numbers.begin(),numbers.end());
            answer++;
        }
        if(in > k && numbers[in] > target || (in < k && numbers[in] < target)){
            cout << -1 << endl;
            continue;
        }
        ...
        pos = findThePosition(numbers,target,in);
        cout << (abs(pos - in) + answer)  << endl;
    }
    return 0;
}