...
int findThePosition(vector<int> numbers,int target,int ){
...
int main(){
   ...
   testcase(){
        ...
        int pos = findThePosition(numbers,target,),answer = 0;
        if(numbers[pos] != target){
            numbers[k] == target;
            sort(numbers.begin(),numbers.end());
            answer++;
        }
        if( > k && numbers[] > target || ( < k && numbers[] < target)){
            cout << -1 << endl;
            continue;
        }
        if(numbers[] == target){
        ...
        pos = findThePosition(numbers,target,);
        cout << (abs(pos - ) + answer)  << endl;
    }
    return 0;
}