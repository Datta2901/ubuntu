...
int findThePosition(vector<int> numbers,int target,int reqi){
...
int main(){
    EXECUTE_FASTER
   testcase(){
        int size,target,reqi,k;
        cin >> size >> target >> reqi >> k;
        reqi -= 1,k -= 1;
        vector<int>numbers;
        ...
        int pos = findThePosition(numbers,target,reqi),answer = 0;
        if(numbers[pos] != target){
            numbers[k] == target;
            sort(numbers.begin(),numbers.end());
            answer++;
        }
        if(reqi > k && numbers[reqi] > target || (reqi < k && numbers[reqi] < target)){
            cout << -1 << endl;
            continue;
        }
        if(numbers[reqi] == target){
            cout << answer << endl;
            continue;
        } 
        pos = findThePosition(numbers,target,reqi);
        cout << (abs(pos - reqi) + answer)  << endl;
    }
    return 0;
}