...
int main(){
   ...
   testcase(){
        ...
        int pos = findThePosition(numbers,target,required),answer = 0;
        if(numbers[pos] != target){
            numbers[k] == target;
            sort(numbers.begin(),numbers.end());
            answer++;
        }
        if(required > k && numbers[required] > target || (required < k && numbers[required] < target)){
            cout << -1 << endl;
            continue;
        }
        if(numbers[required] == target){
            cout << answer << endl;
            continue;
        } 
        pos = findThePosition(numbers,target,required) + 1;
        cout << (abs(pos - required) + answer)  << endl;
    }
    return 0;
}