...
int FindThePosition(int a[], int n, int x, int i) {
...
int main(){
   ...
   testcase(){
       int size, target, i, k;
        ...
        cin >> a[i];
        sort(a, a+size);
        int index = FindThePosition(a, size, target, i);
        int count = 0;
        if(a[index] != target){
            a[k-1] = target;
            sort(a, a+size);
        ...
        if(i < k && a[i-1] < target ||i > k && a[i-1] > target) {
            cout << -1 << endl;
        return;
        }
    index = FindThePosition(a, size, target, i) + 1;
    cout << abs(i-index)+count << endl;
   }
   return 0;
}