...
int FindThePosition(vector<int>numbers, int x, int index) {
...
int main(){
   ...
   testcase(){
        ...
        inc_sort(numbers);
        int index = FindThePosition(numbers,target,index);
        int answer = 0;
        if(numbers[index] != target){
            numbers[smallest-1] = target;
            inc_sort(numbers);
            answer++;
        }
        ...
        if((index < smallest && numbers[index-1] < target) ||(index > smallest && numbers[index-1] > target)){
            cout << -1 << endl;
            continue;
        }
    index = FindThePosition(numbers,target,index) + 1;
    cout << abs(index-index)+answer << endl;
   }
   return 0;
}