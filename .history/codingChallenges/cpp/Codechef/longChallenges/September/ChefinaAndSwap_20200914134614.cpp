...
#define printVector(vect,dataStruct) for(vector<dataStruct> :: iterator it = vect.begin() + 1; it !=vect.end();it++){cout << *it;}
...
int main(){
   EXECUTE_FASTER
      ...
      testcase(){
         cin >> n;
         vector<int> number (n + 1),answer(n + 1);
         for(int i = 1; i <= n; i++){
            number[i] = i;
         }
         int sum = (n*(n + 1)) / 2;
