...
vector<int> calculateEqualDistance(vector<int> velocity,int max,int size){
    set<ll> Equal;
    vector<int> time;
    for(int i = 0; i < 4 * max; i++){
        for(int j = 1; j <= size; j++){
            ...
            time.pb(1 + (size - (Equal.size()))); Equal.clear();
        } 
...
int main(){
   EXECUTE_FASTER
    testcase(){
        ...
        time = calculateEqualDistance(Velocity,max,size);
