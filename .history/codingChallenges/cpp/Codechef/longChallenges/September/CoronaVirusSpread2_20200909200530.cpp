...
typedef map<box,int> boxSum ;
 ...
 map<int,set<ll>>  calculateEqualDistance(vector<int> velocity,int max,int size){
    map<int,set<ll>> Equal;
...
int main(){
   EXECUTE_FASTER
    testcase(){
        ...
        map<int,set<ll>>  time = calculateEqualDistance(Velocity,max,size);

        sort(time.begin(),time.end());
        if(time[time.size() - 1] == size){
            cout << size << " " << size << endl;
        }else{
            cout << time[0]  << " " << time[time.size() - 1] << endl;
        }
        printVector(time,int);
    }
   return 0;
}