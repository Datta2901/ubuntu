...
vector<vector<int>> calculateDistance(vector<int>velocity,int product,int size){
     vector< vector<int> > arr(product,vector<int> (size + 1, 0)); 
...
int main(){
   EXECUTE_FASTER
    testcase(){
        ...
        vector< vector<int> > arr( product,vector<int> (size + 1, 0)); 
        arr = calculateDistance(Velocity,product,size);
        map<int,set<int>>number;

        // for(int i = 0; i < product * max; i++){
        //     for(int j = 1; j <= size; j++){
        //         cout << arr[i][j] << " ";
        //     }
        //     cout << endl;
