...
int main(){
   ...
   testcase(){
        ...
        cin >> size;
        vec(int) dishes;
        vectorInput(dishes,size);
        int answer = 0,difference = 0;
        inc_sort(dishes);
        for(int i = size - 1; i >= 0 size; i = i + 2){ 
            answer += max(max(dishes[i],dishes[i - 1]) - difference,min(dishes[i],dishes[i - 1]));
            difference = abs(dishes[i] - dishes[i - 1]); 
        }
        if(size % 2 == 1){
            answer += abs(dishes[size - 1] - difference);
        }
        cout << answer << endl; 
   }
   return 0;
