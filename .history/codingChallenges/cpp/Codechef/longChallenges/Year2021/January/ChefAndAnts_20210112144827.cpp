...
#define printMapOfVector(name,dataStr) for(map<dataStr ,vector<dataStr> > :: iterator it = name.begin(); it != name.end(); it++){cout << it->first << "  : ";for(ve ip = it->second.begin() ; ip != it->second.end(); ip++){  cout << *ip << " ";}cout << endl;}
...
long long int SubTask2(int n){
    ...
    // collisions on line
    for(ve it = positions.begin(); it != positions.end(); it++){
            vector<int>pos,neg; // store distances at respective sides
        for(ve itr = it->begin(); itr != it->end(); itr++){
            if(*itr > 0){
