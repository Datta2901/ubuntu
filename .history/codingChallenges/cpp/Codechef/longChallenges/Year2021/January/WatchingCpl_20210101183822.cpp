...
int getMinimumCount(vec(int) numbers,int target){
    int count = 0,control = 0;
    int sum = numbers[numbers.size];
    vector<int>::iterator it;
    while(control <= 2){
        it = upper_bound(numbers.begin(),numbers.end(),sum);
        if((it-numbers.begin() + 1) != numbers.size()){
            sum = sum - *it;
            count++;
            numbers.erase(it);
            if(sum <= 0){
...
int main(){
    ...
    testcase(){
        ...
        cout << getMinimumCount(numbers,RequiredSum) << endl;
    }
   return 0;
}