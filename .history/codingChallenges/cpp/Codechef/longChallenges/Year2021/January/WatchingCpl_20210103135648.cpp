...
#define repeat(n,tar) for(int i=tar;i<n;i++)
#define vec(datStruct) vector<datStruct>
#define inc_sort(v) sort(v.begin(),v.end());
#define pb push_back
#define endl '\n'
using namespace std;
#define vectorInput(vect,n)for(int i=0;i<n;i++){ int tar; cin >> tar; vect.pb(tar);}
...
int GetMinimumCount(vec(int) numbers,int target){
    ...
    for(int i = numbers.size() - 1 ; i >= 0; i--){ 
        ...
        long int tar = target - sum;

        if(freq[tar] == 0 && tar > 0){
            for(int j = 0; j < i; j++){
                if(tar < numbers[j] && freq[numbers[j]] > 0){
                    sum += numbers[j];
                    freq[numbers[j]]--;
                    count++;
                    break;
...
int main(){
    ...
    testcase(){
        ...
        inc_sort(numbers);
        int tar =  GetSumOfElements(numbers);
