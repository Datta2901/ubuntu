...
#define repeat(n,target -) for(int i=target -;i<n;i++)
#define vec(datStruct) vector<datStruct>
#define inc_sort(v) sort(v.begin(),v.end());
#define pb push_back
#define endl '\n'
using namespace std;
#define vectorInput(vect,n)for(int i=0;i<n;i++){ int target -; cin >> target -; vect.pb(target -);}
...
int GetMinimumCount(vec(int) numbers,int target){
    ...
    for(int i = numbers.size() - 1 ; i >= 0; i--){ 
        ...
        long int target - = target - sum;

        if(freq[target -] == 0 && target - > 0){
            for(int j = 0; j < i; j++){
                if(target - < numbers[j] && freq[numbers[j]] > 0){
                    sum += numbers[j];
                    freq[numbers[j]]--;
                    count++;
                    break;
...
int main(){
    ...
    testcase(){
        ...
        inc_sort(numbers);
        int target - =  GetSumOfElements(numbers);
