...
#define repeat(n,target - su) for(int i=target - su;i<n;i++)
#define vec(datStruct) vector<datStruct>
#define inc_sort(v) sort(v.begin(),v.end());
#define pb push_back
#define endl '\n'
using namespace std;
#define vectorInput(vect,n)for(int i=0;i<n;i++){ int target - su; cin >> target - su; vect.pb(target - su);}
...
int GetMinimumCount(vec(int) numbers,int target){
    ...
    for(int i = numbers.size() - 1 ; i >= 0; i--){ 
        ...
        long int target - su = target - sum;

        if(freq[target - su] == 0 && target - su > 0){
            for(int j = 0; j < i; j++){
                if(target - su < numbers[j] && freq[numbers[j]] > 0){
                    sum += numbers[j];
                    freq[numbers[j]]--;
                    count++;
                    break;
                }
                // else if(freq[sum - numbers[j]] > 0 && freq[numbers[j]] > 0){
                //     count += 2;
                //     freq[numbers[j]]--;
                //     freq[sum - numbers[j]]--;
                //     sum = target;
...
int main(){
    ...
    testcase(){
        ...
        inc_sort(numbers);
        int target - su =  GetSumOfElements(numbers);
