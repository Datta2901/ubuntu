...
#define repeat(n,target - sum) for(int i=target - sum;i<n;i++)
#define vec(datStruct) vector<datStruct>
#define inc_sort(v) sort(v.begin(),v.end());
#define pb push_back
#define endl '\n'
using namespace std;
#define vectorInput(vect,n)for(int i=0;i<n;i++){ int target - sum; cin >> target - sum; vect.pb(target - sum);}
...
int GetMinimumCount(vec(int) numbers,int target){
    ...
    for(int i = numbers.size() - 1 ; i >= 0; i--){ 
        ...
        long int target - sum = target - sum;
...
int main(){
    ...
    testcase(){
        ...
        inc_sort(numbers);
        int target - sum =  GetSumOfElements(numbers);
