    ...
    for(int i = numbers.size() - 1 ; i >= 0; i--){ 
        ...
        if(freq[target - sum] == 0 && target - sum > 0){
            for(int j = 0; j < i; j++){
                ...
                //     if((target - sum - numbers[j]) == numbers[j] && freq[numbers[j]] >= 2){
                //         sum = target;
                //         freq[numbers[j]] -= 2;
                //         count += 2;
                //     }else{
...
int main(){
    ...
    testcase(){
        ...
        inc_sort(numbers);
        int sum =  GetSumOfElements(numbers);
        int a = GetMinimumCount(numbers,RequiredSum);
        int b = GetMinimumCount2(numbers,RequiredS)
        if(a == -1){
