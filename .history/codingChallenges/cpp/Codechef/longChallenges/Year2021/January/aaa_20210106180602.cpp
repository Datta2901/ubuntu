...
int solve(){
    ...
    for(int i = size - 2; i >= 0; i--){
        ...
        while(i1 != divide.end()){
            int p =(*i1);
            temporary.insert(p);
            temporary.insert(nu[i]);
            temporary.insert(p + nu[i]);
            if(((p + nu[i]) >= target) && (sum - p - nu[i] >= target)){
                z = size - i;
                break;
            }
            if((nu[i] >= target) && (sum - nu[i]) >= target){
                z = size - i;
                break;
            }
            i1++;
