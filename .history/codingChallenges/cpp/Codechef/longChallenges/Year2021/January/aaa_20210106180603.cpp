...
int solve(){
    ...
    for(int i = size - 2; i >= 0; i--){
        ...
        while(i1 != divide.end()){
            int p =(*i1);
            temporary.insert(p);
            temporary.insert(number[i]);
            temporary.insert(p + number[i]);
            if(((p + number[i]) >= target) && (sum - p - number[i] >= target)){
                z = size - i;
                break;
            }
            if((number[i] >= target) && (sum - number[i]) >= target){
                z = size - i;
                break;
            }
            i1++;
