...
int GetMinimumCount(vector<int> numbers,int target,int size){
    int sum = 0;
    set<int>divide;
    divide.insert(numbers[size - 1]);int answer = -1;
    sum = numbers[size - 1];
    ...
    for(int i = size - 2; i >= 0; i--){
        ...
        while(it != divide.end()){
            ...
            temporary.insert(value + numbers[i]);
            if( (sum - numbers[i]) >= target && (numbers[i] >= target)){
                answer = size - i;
                break;
            }
            if((sum - value - numbers[i] >= target) && ((value + numbers[i]) >= target)){
                answer = size - i;
                break;
            }
            it++;
...
int solve(){
    ...
    int answer = GetMinimumCount(numbers,target,size);
    return answer;      
}
