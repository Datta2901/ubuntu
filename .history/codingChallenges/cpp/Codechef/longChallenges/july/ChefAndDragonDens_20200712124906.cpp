...
int main(){
    ...
    vector<pair<long int,long int> > heights,hei;
    int size;
    cin >> size;
    loop(size){
        long int a;
    ...
    while(Queries--){
        ...
        if(a == 1){
            ...
            long int minimum = min(b,c),max;
            vector<pair<long int,long int> > :: iterator  it = find_if(heights.begin(),heights.end(),boolequal;
        }
