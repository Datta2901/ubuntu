...
int main(){
    ...
    while(Queries--){
        ...
        if(a == 1){
            ...
            long int minimum = min(hei[b],hei[c]);
            if(minimum == hei[b]){
                findele = b;
            }else{
                findele = c;vector<pair<long int,long int> > :: iterator  it = find_if(heights.begin(),heights.end(),isEqual);
