...
long int sum(vector<pair<long int,long int> > heights,vector<long int> taste, long int a,long int b,long int ha,long int ){
    long int sum = 0;
    for(vector<pair<long int,long int> > :: iterator it = heights.begin() + min(a,b); it != heights.end(); it++){
        if(it->second >= min(a,b) && it-> second <= max(a,b)){
            sum +=  it->first;
        }
    }
