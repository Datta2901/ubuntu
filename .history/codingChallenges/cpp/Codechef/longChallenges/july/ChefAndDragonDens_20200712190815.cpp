...
long int sum(vector<pair<long int,long int> > heights,vector<long int> taste, long int a,long int b,long int ha,long int hb){
    ...
    for(vector<pair<long int,long int> > :: iterator it = heights.begin() + min(a,b); it != heights.end(); it++){
        ...
        if(it->second - 1 >= min(a,b) && it-> second - 1 <= max(a,b) && ha >= it->first  && hb >= it->first){
            sum +=  taste[(it->second) - 1];
        }else if(it->second - 1 >= min(a,b) && it-> second - 1<= max(a,b) && (ha < it->first || hb < it->first)){
            cout << ""
            cout << it -> first << " " << it ->second << endl;
            return -1;
        }
...
int main(){
    ...
    while(Queries--){
        ...
        if(a == 1){
            ...
            ll answer;
            cout << "it before " << it->second-1 << " " << itr ->second-1 <<" " <<it->first << " " << itr->first << endl;
            answer = sum(heights,taste,it->second-1,itr->second-1,it->first,itr->first); 
            if(answer == -1){
                cout << -1 << endl;
            }else{
                cout << answer << endl;
