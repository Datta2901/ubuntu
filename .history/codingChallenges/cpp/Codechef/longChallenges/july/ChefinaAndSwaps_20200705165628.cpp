...
int main(){
    testcase(){
        ...
        bool flag = true;
        for(unordered_map<long int ,long int> :: iterator it = freA.begin(); (it != freA.end()) || (freA.size() == 1); it++){
            if(freA.size() == 1){
                it = freA.begin();
                flag = false;
            }
            ...
            if(flag = false;freA.size() == 1){
                
                break;
            }
        }

        // unordered_map<long int ,long int > :: iterator it = freA.begin();
        // if(freA.size() == 1){
        //     if(freB[it->first] == it->second){
        //         cout << it->first << " " << it->second << endl;
        //         freA.erase((it));
