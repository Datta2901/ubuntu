...
#define printVectofPairs(name,dataStr) for(vector<pair<dataStr,dataStr> >:: iterator it = name.begin(); it != name.end(); it++){ cout << it->first << " " << it->}
...
int main(){
    testcase(){
        ...
        for(unordered_map<long int ,long int> :: iterator it = freA.begin(); it != freA.end(); i++){
          ...
          sort(B.begin(),B.end(),greater<long int,long int>());     // descending order based on keys;
