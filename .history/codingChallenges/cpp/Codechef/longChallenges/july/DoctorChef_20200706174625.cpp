...
long int RequiredDays(vector<long int> infected,long int cures,int nearer,unordered_map<longint,long int> freq){
    long int answer = 1;
    for(int i = nearer; i < infected.size();){
        cures = min(cures,infected[i]);
        if(cures >= infected[i]){
            long int a = infected[i];
            answer += (freq[a] - 1);
            i++;
        }else{
            answer++;
...
int main(){
    ...
    testcase(){
        ...
        vector<long int > :: iterator it = lower_bound(infected.begin(),infected.end(),cures,freq);
        cout <<" lower_bound "<< (it-infected.begin()) << " value is " << *it  << endl;
        long int days = RequiredDays(infected,cures,(it- infected.begin()));
