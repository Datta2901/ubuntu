...
long int longest(vector<long int> height,vector<long int> taste,long int a,long int b){
    long long int sum = 0;
    vector<long int> lis; 
    ...
    for(int i = a; i <= b; i++){
        ...
        if((it == lis.end() && height[i] <= height[b]) || height[i] == *it){
...
long int requiredSum(vector<long int> height,vector<long int> taste,long int a,long int b){

    if(height[a] > height[b]){
        if(b > a){
            return longest(height,taste,,b);
        }
        return longest(height,taste,a,b);
    }else if(height[a] == height[b]){
        return taste[a] + taste[b];
    }else{
        if(a > b){
            return longest(height,taste,b,a);     
        }
        return longest(height,taste,a,b);
    }  
}
...
int main(){
    ...
    while(Queries--){
        ...
        if(a == 1){
           ...
           answer = requiredSum(hei,taste,b - 1,c - 1);
        }
        cout << answer << endl;
    }
