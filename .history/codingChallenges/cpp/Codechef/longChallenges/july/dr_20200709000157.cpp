...
long long int RequiredDays(vector<long long int> infected,long long int cures,int nearer){
  long long int extraDays = 0,count = 0;
...
int main(){
    ...
    testcase(){
        ...
        long long int days;
            if(2 * infected[it-infected.begin() - 1] >= min(infected[it-infected.begin()],cures) && (it - infected.begin()) != 0){
                cures = 2 * infected[it-infected.begin()- 1];
                days = RequiredDays(infected,cures,(it- infected.begin() - 1));
                // cout << cures << endl;
                cout << size + days + 1 << endl;
                continue;
            }
            else{
