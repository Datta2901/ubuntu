...
ll minAbsoluteIndex(vector<ll> infected,ll cures,ll nearer){
    ll minimum = cures,index = 0;
    for(ll i = nearer ; i < infected.size();i++){

        if(minimum > min(cures,infected[i])){
            index = i;
        }
    }
    return index;
...
long long int RequiredDays(vector<long long int> infected,long long int cures){
  long long int extraDays = 0;
  
  for(int i = minAbsoluteIndex(infected,cures,0) ; i < infected.size();){
      if(cures >= infected[i]){
          cures = 2 * infected[i];
        //   cout << cures << " " ;
...
int main(){
    ...
    //    days = RequiredDays(infected,cures,(it- infected.begin()));
         days = RequiredDays(infected,cures);
