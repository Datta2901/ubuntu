...
int main(){
    ...
    testcase(){
        ...
        loop(4 * size - 1){
            ...
            (firstAxis.count(xcoordinate) > 0)  ? firstAxis.erase(xcoordinate) : ;
            // if the element is already there then remove it , otherwise add the element
            (secondAxis.count(ycoordinate) > 0) ? secondAxis.erase(ycoordinate) : ;secondAxis.insert(ycoordinate);
            //if the element is already there then remove it , otherwise add the element
            (firstAxis.count(xcoordinate) == 0) ? firstAxis.insert(xcoordinate) : ;
             (secondAxis.count(ycoordinate) > 0) ? secondAxis.erase(ycoordinate) :
        }
