...
long int RequiredDays(vector<long int> infected,long int cures){
    ...
    for(int i = it-infected.begin() ; i < infected.size();){
        if(cures >= infected[i]){
            ...
            vector<long int> :: iterator it = lower_bound(infected.begin()+ (it - infected.begin()+1),infected.end(),cures);
            cout << it - infected.begin() <<  " " << infected.begin()+ (it - infected.begin()+1) <<endl;
            count++;
            i = it - infected.begin();
        }else{
