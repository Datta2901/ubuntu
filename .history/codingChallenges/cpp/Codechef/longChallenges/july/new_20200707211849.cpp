...
long int RequiredDays(vector<long int> infected,long int cures,long int size){
    ...
    for(int i = it-infected.begin() ; i < infected.size();){
        ...
        if(cures >= infected[i]){
            cures = 2 * cures;
            cout << "previous iterator is " <<  i << " and value at " << *it << " ";\            it  = lower_bound(infected.begin() + i + 1,infected.end(),cures);
            
            cout << "next iterator is " << it - infected.begin() << " and value at "  << *it <<  endl;
            cout << "vector is " ; 
            printVect(infected,long int);
            i  = it - infected.begin(); 
            answer++;
