...
ll find(v<ll>taste,v<ll> heights,ll start,ll end){
    ...
    if(find(taste,heights,start + 2,end) == -1 && taste[start] + find(taste,heights,start + 1,end) == -1){
        return -1;
    }
    if(taste[start] + find(taste,heights,start + 1,end) == -1){
        return  find(taste,heights,start + 2,end);
    }
    if(find(taste,heights,start + 2,end)){
        return find(taste,heights,start + 1,end);
    }
    find(taste,heights,start + 2,end)
    maximum = max(, taste[start] + find(taste,heights,start + 1,end));
