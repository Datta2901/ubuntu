...
ll from(v<ll>taste,v<ll> heights,ll start,ll end){
    ll maximum = 0 ;
    if(start == end){
        return taste[end];
    }
    ...
    ll a = from(taste,heights,start + 2,end),b = taste[start] + from(taste,heights,start + 1,end);
    if(a == -1 && b == -1){
        return -1;
    }
    if(a == -1){
...
int main(){
    ...
    while(Queries--){
        ...
        if(a == 1){
           ...
           answer =  from(taste,height,b - 1 ,c - 1);
           cout << answer << endl;
        }
    }
        return 0;
