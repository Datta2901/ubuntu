...
ll fromstart(v<ll>taste,v<ll> heights,ll start,ll end){
    ll maximum = 0 ;
    if(start == end){
        return taste[end];
    }
    ...
    ll a = fromstart(taste,heights,start + 2,end),b = taste[start] + fromstart(taste,heights,start + 1,end);
    if(a == -1 && b == -1){
        return -1;
    }
    if(a == -1){
...
int main(){
    ...
    while(Queries--){
        ...
        if(a == 1){
           ...
           answer =  fromstart(taste,height,b - 1 ,c - 1);
           cout << answer << endl;
        }
    }
        return 0;
