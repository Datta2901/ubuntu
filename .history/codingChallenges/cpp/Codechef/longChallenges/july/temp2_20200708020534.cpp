...
long int RequiredDays2(vector<long int> infected, long int vaccine, long int original) {
    auto it = lower_bound(infected.begin(), infected.end(), vaccine/2);

    if (*it >= vaccine/2) {
        return RequiredDays2(infected, *it);
    } else{
        return 1 + RequiredDays2(infected, vaccine);
    }
}
