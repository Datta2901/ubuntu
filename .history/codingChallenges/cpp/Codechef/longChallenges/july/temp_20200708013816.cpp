...
long int RequiredDays2(vector<long int> infected, long int vaccine) {
    ...
    long int cityInfCount=0 , vaccineCount=0;
    while(vaccine < infected[infected.size() - 1]) {
        ...
        if (lb == -1){
            ...
            cityInfCount = daystofill(infected[infected.size() - 1], infected[lb]);
            vaccineCount = daystofill(infected[infected.size() - 1], vaccine);
            if (1+ vaccineCount <= 1 + cityInfCount)
