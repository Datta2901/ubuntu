...
void solve(){
    ...
    int Delivered,team2,team3,team4;
    cin >> Delivered >> team2 >> team3 >> team4;
    unordered_map<pair<int,int>,int>common;
    unordered_map<int,set<string>> Ingred;
    for(int i = 1; i <= Delivered; i++){
        ...
        for(int j = 0; j < size; j++){
            string type;
            cin >> type;
            Ingred[i].insert(type);
        }            
    }

    for(auto it = Ingred.begin(); it != Ingred.end(); it++){
        cout << *it << "-"
        for(auto itr = Ingred[*it].begin(); itr != Ingred[*it].end(); itr++){
            cout << *itr << " ";
        }
    }
}
