...
#define loopv(i, n) for ( i = 0; i < n; i++)
#define FOR(i, x, y) for ( i = x; i < y; i++)
#define DEC(i, x, y) for ( i = x; i >= y; i--)
//precision
...
#define dec_sort(v) sort(v.begin(),v.end(),greater<>());
#define EXECUTE_FASTER ios::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
#define prMap(map,name,dataStr,data) for(map<dataStr,data>:: iterator it = name.begin(); it != name.end(); it++){ cout << it->first << " " << it->second << endl;}
#define prVector(vect,dataStruct) for(vector<dataStruct> :: iterator it = vect.begin(); it !=vect.end();it++){cout << *it << " ";}
#define prVectorofPairs(name,dataStr) for(vector<pair<dataStr,dataStr> >:: iterator it = name.begin(); it != name.end(); it++){ cout << it->first << " " << it->second << endl;}
#define prMapOfVector(name,dataStr) for(map<dataStr ,vector<dataStr> > :: iterator it = name.begin(); it != name.end(); it++){cout << it->first << "  : ";for(auto ip = it->second.begin() ; ip != it->second.end(); ip++){  cout << *ip << " ";}cout << endl;}
#define vectorInput(vect,n)for( i=0;i<n;i++){  a; cin >> a; vect.pb(a);}
