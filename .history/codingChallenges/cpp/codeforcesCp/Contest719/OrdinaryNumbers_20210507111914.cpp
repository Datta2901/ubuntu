...
#define loopv(i, n) for (long long  i = 0; i < n; i++)
#define FOR(i, x, y) for (long long  i = x; i < y; i++)
#define DEC(i, x, y) for (long long  i = x; i >= y; i--)
//precision
...
#define dec_sort(v) sort(v.begin(),v.end(),greater<long long >());
#define EXECUTE_FASTER ios::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
#define prlong long Map(map,name,dataStr,data) for(map<dataStr,data>:: iterator it = name.begin(); it != name.end(); it++){ cout << it->first << " " << it->second << endl;}
#define prlong long Vector(vect,dataStruct) for(vector<dataStruct> :: iterator it = vect.begin(); it !=vect.end();it++){cout << *it << " ";}
#define prlong long VectorofPairs(name,dataStr) for(vector<pair<dataStr,dataStr> >:: iterator it = name.begin(); it != name.end(); it++){ cout << it->first << " " << it->second << endl;}
#define prlong long MapOfVector(name,dataStr) for(map<dataStr ,vector<dataStr> > :: iterator it = name.begin(); it != name.end(); it++){cout << it->first << "  : ";for(auto ip = it->second.begin() ; ip != it->second.end(); ip++){  cout << *ip << " ";}cout << endl;}
#define vectorInput(vect,n)for(long long  i=0;i<n;i++){ long long  a; cin >> a; vect.pb(a);}
...
// Binary search
long long  binarySearch(long long  arr[], long long  l, long long  r, long long  key) { 
    while (l <= r) { 
...
void isPrime(){
  ...
  for(long long  p=2;p*p<mxN;p++){
    if(prime[p]){
      for(long long  i=p*p;i<mxN;i+=p){
        prime[i]=false;
      }
    }
  }
...
void solve(){
    ...
    long long  count = 0;
    bool flag = true;
    for(long long  i = 11; i > 0; i = 10 * i + 1){
