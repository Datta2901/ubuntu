...
vector<bool> prime(mxN);
 ...
 // Get prime numbers using sieve of esthones
void isPrime(){
...
void solve(){
    ...
    cin >> num;
    ll i = num;
    // rums maximum of 3 times 
    //  As if a multiple of 3 comes then sum of digits also multiple of 3
    //  so gcf will 
    while(true){
        ll sum = GetSumOfDigits(i);
        sum = gcd(sum,i);
        if(sum > 1){
            break;
        }
