...
bool isPowerOf2(ll number){
    return (floor(log2(number)) == (log2(number)));
}
...
void isPrime(){
  ...
  for(int require=2;require*require<mxN;require++){
    if(prime[require]){
      for(int i=require*require;i<mxN;i+=require){
        prime[i]=false;
      }
    }
  }
...
void solve(){
    ...
    long long int require,a,b,c,a1,a2,a3;
    cin>>require>>a>>b>>c;
    if(require%a==0||require%b==0||require%c==0){
            cout<<0<<endl;
            continue;
    }
    a1=((require/a)+1)*a-require;
    a2=((require/b)+1)*b-require;
    a3=((require/c)+1)*c-require;
