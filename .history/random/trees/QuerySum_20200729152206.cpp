...
using namespace std;
#define m 10
int t[4 * m];// maximum size of the segment tree is 4 * size of given  array

// building of segment tree in O(n) 
// building the the tree 
void build(int a[],int v,int low,int tr){
...
int sum(int v,int low,int tr,int l,int r){
    if(l > r){
        return 0;
    }
    if(l == low && r == tr){
    ...
    int tm = (low + tr) / 2;
    return sum(2 * v,low,tm,l,min(tm,r)) + sum(2 * v + 1,tm + 1,tr,max(tm + 1,l),r); 
}
