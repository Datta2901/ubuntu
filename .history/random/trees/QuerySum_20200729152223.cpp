...
using namespace std;
#define m 10
int t[4 * m];// maximum size of the segment highee is 4 * size of given  array
...
void build(int a[],int v,int low,int high){
    ...
    if(low == high){
        ...
        int tm = (low + high)/2;
        build(a,2 *v,low,tm); 
        build(a,2 * v + 1,tm + 1,high);
        t[v] = t[2 * v] + t[2 *v + 1]; 
    }  
}

// can be calculated in log(n)
int sum(int v,int low,int high,int l,int r){
    if(l > r){
        return 0;
    }
    if(l == low && r == high){
    ...
    int tm = (low + high) / 2;
    return sum(2 * v,low,tm,l,min(tm,r)) + sum(2 * v + 1,tm + 1,high,max(tm + 1,l),r); 
}
