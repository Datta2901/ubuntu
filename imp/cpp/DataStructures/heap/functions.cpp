...
int main(){
    ...
    pattern.pop_back();
    cout << "After deleting the maximum element heap becomes ";
    printHeap(pattern);
    ...
    cout << "After Adding the maximum element heap becomes ";
    printHeap(pattern);
    
    cout << "Maximum element is " << pattern.front() << endl;
    
    sort_heap(pattern.begin(),pattern.end());
    cout << "After sorting heap becomes : ";
    printHeap(pattern);

    make_heap(pattern.begin(),pattern.end());       // to convert into a heap
    is_heap(pattern.begin(), pattern.end()) ? cout << "The container is heap " : cout << "The container is not heap"; 
    cout << endl; 
    auto it = is_heap_until(pattern.begin(), pattern.end()); 
    cout << "The heap elements in container are : "; 
    for (auto it1=pattern.begin(); it1 != it; it1++){
         cout << *it1 << " ";
    } 
    cout << endl;   
    return 0;
