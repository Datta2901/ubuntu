...
#define EXECUTE_FASTER           \
    ios::sync_with_stdio(false); \
    cin.tie(NULL);               \
    cout.tie(NULL);
#define printMap(map, name, dataStr, data)                                       \
    for (map<dataStr, data>::iterator it = name.begin(); it != name.end(); it++) \
    {                                                                            \
        cout << it->first << " " << it->second << endl;                          \
    }
#define printVector(vect, dataStruct)                                            \
    for (vector<dataStruct>::iterator it = vect.begin(); it != vect.end(); it++) \
    {                                                                            \
        cout << *it << " ";                                                      \
    }
#define printVectorofPairs(name, dataStr)                                                     \
    for (vector<pair<dataStr, dataStr> >::iterator it = name.begin(); it != name.end(); it++) \
    {                                                                                         \
        cout << it->first << " " << it->second << endl;                                       \
    }
#define printMapOfVector(name, dataStr)                                                      \
    for (map<dataStr, vector<dataStr> >::iterator it = name.begin(); it != name.end(); it++) \
    {                                                                                        \
        cout << it->first << "  : ";                                                         \
